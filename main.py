...
name_map = {'player': 'player', 'al_lstm': 'player(LSTM)','al_arima': 'player(ARIMA)','al_RNN': 'player(RNN)','al_RF': 'player(Random Forest)','al_XGB': 'player(XGBoost)','no_buy': 'player(No Buy)', 'all_in_tsmc': 'player(All in TSMC)'}
    ...
    try:
        ...
        for user in user_list:
            ...
            cur.execute(
                ...
                (0, user, json.dumps(holdings), cash)
            )
...
def next_day_game(day:int, portfolio: Portfolio):
    ...
    try:
        ...
        for row in rows:
            ...
            if user_name == 'no_buy' or user_name == 'all_in_tsmc':
                histories.append({
                    "day": day,
                    "user_name": user_name,
